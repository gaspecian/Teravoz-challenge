...
export class ListCallsComponent implements OnInit {
  ...
  displayedColumns:  string[] = ['call_id', 'type', 'our_number', 'their_number', 'their_number_type', 'start_date', 'finish_date'];

  @ViewChild(MatPaginator, {static: true}) paginator: MatPaginator;

  ngOnInit() {
    this.loading = true;
    this.getCallsService.getCalls().pipe(first()).subscribe(calls => {
        this.loading = false;
        this.calls = calls
    });
  }
